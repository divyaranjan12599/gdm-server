...
import {
  sendAdminUserAddedEmail,
  sendAdminUserRemovedEmail,
  sendUserAddedEmail,
...
export const checkUserEmail = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(401)
        .json({ message: "User Not Found", userExists: false });
    } else {
    ...
    res
      .status(500)
      .json({ message: "User does not exist.", userExists: false });
  }
};
...
export const login = async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      if (passwordMatch) {
        const token = jwt.sign(
          { staffId: staff._id, userId: staff.belongsTo, email, role: "STAFF" },
          process.env.SECRET,
          { expiresIn: "8h" }
        );
        ...
        return res
          .status(200)
          .json({ message: "User verified", user: staffObject, token });
      } else {
    ...
    if (passwordMatch) {
      ...
      return res
        .status(200)
        .json({ message: "User verified", user: user, token });
    } else {
      return res.status(401).json({ message: "Invalid Password" });
    }
  } catch (error) {
...
export const createClient = async (req, res) => {
  try {
    ...
    const existingClientByContact = await Client.findOne({
    ...
    const membershipData = {
      ...
      endDate: endDateGenerator(
        formatDate(membershipStartingDate),
        membershipPeriod
      ),
    ...
    const paymentDetailsData = {
      ...
      paidFor: parseFloat(ptFees) > 0 ? PaidFor.NORMAL_AND_PT : PaidFor.NORMAL,
      belongsTo: req.user.userId,
    };
...
export const getAllMemberships = async (req, res) => {
  const userId = req.user.userId;
  try {
    const memberships = await MembershipDetail.find({ belongsTo: userId })
      .sort({ startDate: 1 })
      .populate("membershipBy");
    if (!memberships) {
...
export const getAllPTMemberships = async (req, res) => {
  const userId = req.user.userId;
  try {
    const memberships = await PTMembershipDetail.find({ belongsTo: userId })
      .sort({ startDate: 1 })
      .populate("ptTo")
      .populate("assignedTo");
    if (!memberships) {
      return res
        .status(404)
        .json({ message: "PT Membership details not found" });
    }
    res.status(200).json(memberships);
  } catch (error) {
    res.status(500).json({ message: error.message });
...
export const getAllMembershipsByClientId = async (req, res) => {
  ...
  try {
    ...
    const memberships = await MembershipDetail.find({
    ...
    if (!memberships || memberships.length === 0) {
      return res
        .status(404)
        .json({ message: "No memberships found for this client" });
    }
...
export const getPtDetailsByStaffId = async (req, res) => {
  const userId = req.user.userId;
  try {
    const { staffId } = req.params;
    const memberships = await PTMembershipDetail.find({
      assignedTo: staffId,
      belongsTo: userId,
    })
      .populate("ptTo", "name")
...
export const updateMembershipByClientId = async (req, res) => {
  ...
  try {
    ...
    if (existingMembership) {
      return res
        .status(400)
        .json({ message: "Client already has an active membership." });
    }
...
export const getAllPaymentDetails = async (req, res) => {
  ...
  try {
    ...
    if (!payments) {
      return res
        .status(404)
        .json({ message: "Client Payment Details not found" });
    }
...
export const updateClientById = async (req, res) => {
  ...
  try {
    const client = await Client.findOneAndUpdate(
      { _id: req.params.id, belongsTo: userId },
      req.body,
      { new: true }
    );
    if (!client) {
...
export const createStaff = async (req, res) => {
  ...
  try {
    ...
    if (existingStaffByContact) {
      return res
        .status(400)
        .json({ message: "Staff with this contact already exists" });
    }
...
export const getAllStaff = async (req, res) => {
  const userId = req.user.userId;
  try {
    const staffs = await Staff.find({ belongsTo: userId }).sort({
      joiningDate: 1,
    });
    if (!staffs) {
      return res.status(404).json({ message: "Staff not found" });
...
export const createEnq = async (req, res) => {
  ...
  try {
    ...
    //     return res.status(400).json({ message: "Staff with this ID already exists" });
    // }
    const attainByStaff = await Staff.findOne({
      _id: attainedBy,
      belongsTo: userId,
    });
    ...
    if (!attainByStaff) {
      return res
        .status(400)
        .json({ message: "Staff attained the client is not exist" });
    }
    ...
    const enqData = {
      // id: newEnqId,
      name: capitalizeEachWord(visitorName),
      contact: phone,
      email: email,
...
export const getAllEnq = async (req, res) => {
  const userId = req.user.userId;
  try {
    const enquiries = await Enquiry.find({ belongsTo: userId })
      .sort({ enquiryDate: 1 })
      .populate("attainBy");
    if (!enquiries) {
      return res.status(404).json({ message: "enquiry not found" });
...
export const addUser = async (req, res) => {
  ...
  try {
    ...
    if (!password) {
      return res
        .status(401)
        .json({ message: "Please provide default password." });
    }

    const hashedPassword = await bcrypt.hash(password, 10);
    const result = await Staff.findByIdAndUpdate(staffId, {
...
export const deleteUser = async (req, res) => {
  ...
  try {
    ...
    user.numberOfUsers = numberOfUsers - 1;
    user.save();
    await sendAdminUserRemovedEmail(
      user.email,
      user.ownerName,
      staff.name,
      staff.email
...
export const updateUserProfile = async (req, res) => {
  ...
  try {
    const updatedUser = await User.findByIdAndUpdate(
      userId,
      { gymTitle, ownerName, email, contact, profilePicture },
